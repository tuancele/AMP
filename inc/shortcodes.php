...
/**
 * inc/shortcodes.php
 * Đăng ký và chứa tất cả các hàm callback cho shortcode.
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

function tuancele_register_shortcodes() {
    add_shortcode('form_dang_ky', 'amp_form_shortcode_function');
    add_shortcode('schema_faq', 'schema_faq_shortcode' );
    add_shortcode('schema_howto', 'schema_howto_shortcode' );
    add_shortcode('amp_slider', 'tuancele_amp_slider_shortcode' );
    add_shortcode('chi_tiet_bds', 'tuancele_bds_details_shortcode' );
    add_shortcode('tinh_lai_suat', 'tuancele_mortgage_calculator_shortcode' );
    add_shortcode('bds_noibat', 'tuancele_featured_properties_shortcode' );
    add_shortcode('tien_ich_xung_quanh', 'tuancele_utilities_wrapper_shortcode' );
    add_shortcode('tien_ich', 'tuancele_utility_item_shortcode' );
    add_shortcode('geo_display', 'tuancele_geo_display_shortcode' );
    add_shortcode('geo_option', 'tuancele_geo_option_shortcode' );
    add_shortcode('amp_product', 'tuancele_amp_product_shortcode' );
    add_shortcode('quang_cao_noi_bo', 'tuancele_internal_ad_shortcode');
    add_shortcode('dang_ky_sdt', 'tuancele_phone_registration_shortcode');
    add_shortcode('amp_imagemap', 'tuancele_amp_imagemap_shortcode');
    
    // [THÊM MỚI] Đăng ký shortcode cho thanh thông báo sự kiện
    add_shortcode('amp_event_bar', 'tuancele_amp_event_bar_shortcode');
}
add_action('init', 'tuancele_register_shortcodes');

// ... (Các hàm shortcode khác giữ nguyên không đổi) ...

// SHORTCODE [form_dang_ky]
function amp_form_shortcode_function($atts) {
    $args = shortcode_atts(['tieu_de' => 'Đăng Ký Tư Vấn Miễn Phí', 'nut_gui' => 'Gửi Thông Tin Ngay'], $atts);
    if (function_exists('get_amp_form_html')) {
        return get_amp_form_html($args);
    }
    return '';
}

// SHORTCODE [schema_faq]
function schema_faq_shortcode( $atts, $content = null ) {
...
// SHORTCODE [schema_howto] - PHIÊN BẢN SỬA LỖI DỨT ĐIỂM
function schema_howto_shortcode( $atts, $content = null ) {
    ...
    // [SỬA LỖI DỨT ĐIỂM]
    // 1. Dùng hàm shortcode_unautop() của WordPress để loại bỏ các thẻ <p> và <br> không mong muốn
    // bao quanh các thẻ [step]. Đây là cách làm chuẩn và an toàn nhất.
    $cleaned_content = shortcode_unautop($content);
    
    // 2. Biểu thức chính quy (Regex) vẫn giữ nguyên để trích xuất dữ liệu từ nội dung đã được làm sạch.
    preg_match_all( '/\[step title=[\"”](.*?)[\"”]\](.*?)\[\/step\]/s', $cleaned_content, $matches );

    if ( empty( $matches[1] ) ) {
        // Nếu vẫn không tìm thấy, trả về lỗi để dễ dàng gỡ rối.
    ...
    for ( $i = 0; $i < count( $matches[1] ); $i++ ) {
        ...
        // Tạo schema với nội dung đã được làm sạch thẻ HTML
        $steps_schema[] = [ 
            '@type' => 'HowToStep', 
        ...
        // Hiển thị nội dung cho người dùng, sử dụng wpautop() để định dạng lại các đoạn văn bản bên trong [step] một cách đẹp đẽ.
    ...
    if( ! empty( $args['total_time'] ) ) {
        $schema['totalTime'] = $args['total_time'];
    }
...
function tuancele_amp_slider_shortcode( $atts ) {
...
function tuancele_bds_details_shortcode($atts) {
...
function tuancele_mortgage_calculator_shortcode() {
...
function tuancele_featured_properties_shortcode($atts) {
...
function tuancele_utilities_wrapper_shortcode($atts, $content = null) {
...
function tuancele_utility_item_shortcode($atts, $content = null) {
...
function tuancele_get_vietnam_provinces() {
    return [ "hanoi" => "vn-hn", "hcm" => "vn-sg", "danang" => "vn-dn", "haiphong" => "vn-hp", "cantho" => "vn-ct", "angiang" => "vn-44", "bariavungtau" => "vn-43", "backan" => "vn-53", "bacgiang" => "vn-54", "baclieu" => "vn-55", "bacninh" => "vn-56", "bentre" => "vn-50", "binhdinh" => "vn-31", "binhduong" => "vn-57", "binhphuoc" => "vn-58", "binhthuan" => "vn-40", "camau" => "vn-59", "caobang" => "vn-04", "daklak" => "vn-30", "daknong" => "vn-72", "dienbien" => "vn-71", "dongnai" => "vn-39", "dongthap" => "vn-45", "gialai" => "vn-32", "hagiang" => "vn-03", "hanam" => "vn-63", "hatinh" => "vn-23", "haiduong" => "vn-61", "haugiang" => "vn-73", "hoabinh" => "vn-14", "hungyen" => "vn-66", "khanhhoa" => "vn-34", "kiengiang" => "vn-47", "kontum" => "vn-28", "laichau" => "vn-01", "lamdong" => "vn-35", "langson" => "vn-09", "laocai" => "vn-02", "longan" => "vn-41", "namdinh" => "vn-67", "nghean" => "vn-22", "ninhbinh" => "vn-18", "ninhthuan" => "vn-36", "phutho" => "vn-68", "phuyen" => "vn-37", "quangbinh" => "vn-24", "quangnam" => "vn-27", "quangngai" => "vn-29", "quangninh" => "vn-13", "quangtri" => "vn-25", "soctrang" => "vn-52", "sonla" => "vn-05", "tayninh" => "vn-37", "thaibinh" => "vn-20", "thainguyen" => "vn-69", "thanhhoa" => "vn-21", "thuathienhue" => "vn-26", "tiengiang" => "vn-46", "travinh" => "vn-51", "tuyenquang" => "vn-07", "vinhlong" => "vn-49", "vinhphuc" => "vn-70", "yenbai" => "vn-06", ];
}
function tuancele_geo_display_shortcode( $atts, $content = null ) {
    global $geo_options;
    $geo_options = [];
...
function tuancele_geo_option_shortcode( $atts ) {
    global $geo_options;
...
function tuancele_amp_product_shortcode($atts) {
    $post_id = get_the_ID();
    $atts = shortcode_atts([
    ...
    // [FINAL SCHEMA FIX] Bổ sung deliveryTime
    $schema = [
        '@context' => 'https://schema.org/',
        '@type' => 'Product',
        'name' => $atts['name'],
        ...
        'offers' => [
            ...
            'shippingDetails' => [
                ...
                // Thêm trường 'deliveryTime' để khai báo thời gian kích hoạt dịch vụ
                'deliveryTime' => [
                    '@type' => 'ShippingDeliveryTime',
                    'handlingTime' => [
                        '@type' => 'QuantitativeValue',
                        'minValue' => 0, // Tối thiểu 0 ngày
                        'maxValue' => 1, // Tối đa 1 ngày để xử lý
                        'unitCode' => 'DAY'
                    ],
                    'transitTime' => [
                        '@type' => 'QuantitativeValue',
                        'minValue' => 0, // Thời gian vận chuyển là 0
                        'maxValue' => 0,
                        'unitCode' => 'DAY'
                    ]
                ],
...
function tuancele_internal_ad_shortcode($atts) {
...
add_filter('the_content', function($content) {
    if (is_single() && in_the_loop() && is_main_query()) {
        ...
        if (!empty($latest_posts)) {
            $ad_code = do_shortcode('[quang_cao_noi_bo id="' . $latest_posts[0]->ID . '"]');
...
function tuancele_phone_registration_shortcode($atts) {
    $args = shortcode_atts([
        'tieu_de' => 'Để lại số điện thoại, chúng tôi sẽ gọi lại ngay!',
        'nut_gui' => 'Yêu Cầu Gọi Lại'
    ], $atts);
    ...
    <div class="amp-form-container amp-form-phone-only">
        ...
        <form method="POST" target="_top" action-xhr="<?php echo $form_action_url; ?>">
            ...
            <div submit-success>
                <div class="form-feedback form-success"><span>Yêu cầu thành công! Chúng tôi sẽ sớm liên hệ với bạn.</span></div>
            </div>
            <div submit-error>
                <div class="form-feedback form-error"><span>Đã có lỗi xảy ra, vui lòng thử lại!</span></div>
            </div>
        </form>
    </div>
    <?php
...
function tuancele_amp_imagemap_shortcode($atts) {
    // 1. Phân tích ID
    ...
    // 3. Xử lý dữ liệu Hotspot
    // Sửa regex để chấp nhận cả tiếng Việt và các ký tự khác
    ...
    <div class="amp-css-imagemap-wrapper" data-map-id="<?php echo $map_id; ?>" style="position: relative;">
        ...
        <?php foreach ($hotspot_data as $data) : 
            ...
            if ($mode === 'popup') {
                ...
                $allowed_html = [
                    'h1' => [], 'h2' => [], 'h3' => [], 'h4' => [], 'h5' => [], 'h6' => [], 'p'  => ['style' => []], 'br' => [], 'strong' => [], 'b' => [], 'em' => [], 'i' => [],
                    'ul' => [], 'ol' => [], 'li' => [], 'a' => ['href' => [], 'title' => [], 'target' => []], 'img' => ['src' => [], 'alt' => [], 'width' => [], 'height' => [], 'style' => []],
                ];
                $safe_content = wp_kses($data['content'], $allowed_html);
                
                $hotspot_state_data = [
                    'title' => $data['name'],
                    'content_text' => wp_strip_all_tags( $safe_content ),
                    'link' => esc_url( $data['url'] )
                ];
    ...
    <?php if ($mode === 'popup') : ?>
        <amp-state id="currentHotspot">
            <script type="application/json">
            {
                "title": "Thông tin",
                "content_text": "Vui lòng chọn một điểm trên bản đồ hoặc trong danh sách.",
                "link": "#"
            }
            </script>
        </amp-state>
...
/// =========================================================================
// SHORTCODE [amp_event_bar] - (FINAL ACCESSIBILITY & VALIDATION FIX)
// =========================================================================
function tuancele_amp_event_bar_shortcode($atts) {
    $args = [
        'post_type'      => 'event',
        'post_status'    => 'publish',
        'posts_per_page' => -1,
    ...
    <div id="amp-event-bar" class="event-carousel-wrapper">
        <amp-carousel
            ...
            delay="5000"
            loop
            on="slideChange:AMP.setState({ currentEventSlide: event.index })">
            ...
            while ($event_query->have_posts()) : $event_query->the_post(); 
                ...
                // --- Schema Data Logic (No changes here) ---
            ...
            ?>
                <div class="event-slide">
                    ...
                    <?php // [FIX] Phiên bản 1: DIV tương tác, chỉ hiển thị khi slide active ?>
                    <div role="link"
                         tabindex="0"
                         class="event-notification-link"
                         on="tap:AMP.navigateTo(url='<?php echo esc_url($event_url); ?>')"
                         [hidden]="currentEventSlide != <?php echo $slide_index; ?>"
                         <?php if ($slide_index !== 0) echo 'hidden'; ?>>
                        <div class="sonar-icon-wrap">
                            <span class="event-status-icon"><?php echo esc_html($icon); ?></span>
                    ...
                    <?php // [FIX] Phiên bản 2: SPAN không tương tác, chỉ hiển thị khi slide bị ẩn ?>
                    <span class="event-notification-link"
                          [hidden]="currentEventSlide == <?php echo $slide_index; ?>"
                          <?php if ($slide_index === 0) echo 'hidden'; ?>>
                        <div class="sonar-icon-wrap">
                            <span class="event-status-icon"><?php echo esc_html($icon); ?></span>
