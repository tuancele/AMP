<?php
/**
 * inc/event-manager.php
 * Chứa logic Custom Post Type và Meta Box cho tính năng Sự kiện (Event).
 */
...
// =========================================================================
function tuancele_register_event_cpt() {
    $labels = [
    ...
    $args = [
        'labels'                => $labels, 'public' => false, 'show_ui' => true, 'show_in_menu' => true,
...
add_action('init', 'tuancele_register_event_cpt');


// =========================================================================
// 2. META BOX: CẤU HÌNH CHI TIẾT SỰ KIỆN (ĐÃ BỔ SUNG GHI CHÚ)
// =========================================================================
function tuancele_add_event_meta_box() {
    add_meta_box('tuancele_event_details', __('Chi tiết Sự kiện', 'tuancele-amp'), 'tuancele_render_event_meta_box_callback', 'event', 'normal', 'high');
}
add_action('add_meta_boxes', 'tuancele_add_event_meta_box');

function tuancele_render_event_meta_box_callback($post) {
...
// =========================================================================
// 3. LƯU DỮ LIỆU META BOX (GIỮ NGUYÊN)
// =========================================================================
function tuancele_save_event_meta_data($post_id) {
...
add_action('save_post_event', 'tuancele_save_event_meta_data');

// Tải script cho media uploader (Giữ nguyên)
function tuancele_event_admin_scripts($hook) {
...
add_action('admin_enqueue_scripts', 'tuancele_event_admin_scripts');