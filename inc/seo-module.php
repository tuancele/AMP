...
/**
 * inc/seo-module.php
 * Module Class cho việc xử lý SEO, Meta Tags, và Schema JSON-LD.
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

final class AMP_SEO_Module {

    /**
     * Khởi tạo module, đăng ký tất cả các hook và filter liên quan đến SEO.
     */
    public function __construct() {
        // Thẻ Meta (description)
        add_action('wp_head', [ $this, 'amp_meta_tags' ], 1);
        
        // Thẻ Social (Open Graph, Twitter)
        add_action('wp_head', [ $this, 'add_social_meta_tags' ], 2);
        
        // Schema JSON-LD (Graph)
        add_action('wp_footer', [ $this, 'generate_rich_schema' ]);
        
        // Các hàm hỗ trợ Schema Review từ bình luận
        add_action( 'comment_post', [ $this, 'save_comment_rating' ] );
        add_filter( 'comment_text', [ $this, 'generate_review_schema_for_comment' ], 99 );
    }

    /**
     * =========================================================================
     * THẺ META CƠ BẢN (DESCRIPTION) & SOCIAL TAGS
     *
     * =========================================================================
     */
    public function amp_meta_tags() {
        global $post; 
        $description = '';
     ...
     * =========================================================================
     */
    public function generate_rich_schema() {
    ...
    public function generate_review_schema_for_comment( $comment_text ) {
