<?php
/**
 * inc/comments-handler.php
 * Toàn bộ logic cho hệ thống bình luận AMP tùy chỉnh và tích hợp Turnstile.
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

// Giả định rằng các hàm này tồn tại ở nơi khác hoặc được giữ nguyên
// Nếu chúng không tồn tại, bạn cần đảm bảo định nghĩa chúng.
function tuancele_get_turnstile_secret_key() { 
    $options = get_option('tuancele_turnstile_settings', []);
    return $options['secret_key'] ?? '';
}

function tuancele_verify_turnstile_token($token, $ip) {
    $secret_key = tuancele_get_turnstile_secret_key();
...
function tuancele_get_ip_country_code($ip) {
    if (in_array($ip, ['127.0.0.1', '::1', 'Invalid IP'])) return 'VN'; // Giả sử localhost là VN
...
function tuancele_amp_comment_form() {
    ...
    <div id="respond" class="comment-respond">
        ...
        <form action-xhr="<?php echo $form_action_url; ?>" method="post" id="commentform" class="comment-form" target="_top">
            ...
            <?php
            // [SỬA ĐỔI] Thêm trường Nonce vào form để bảo mật
            wp_nonce_field('amp_comment_nonce_action', '_amp_comment_nonce_field');
            ?>
...
function tuancele_handle_amp_comment_submission() {
    // [SỬA ĐỔI] Thêm bước xác thực Nonce ngay từ đầu để chống CSRF
    ...
    $country_code = tuancele_get_ip_country_code($user_ip);
    
    $options = get_option('tuancele_turnstile_settings', []);
    if (!empty($options['site_key'])) {
        $turnstile_token = sanitize_text_field($_POST['cf-turnstile-response'] ?? '');
        if (!tuancele_verify_turnstile_token($turnstile_token, $user_ip)) {
    ...
    $comment_data = [ 
        ...
        'comment_author_url' => '', // Không cần URL
...
add_action('wp_ajax_amp_submit_comment', 'tuancele_handle_amp_comment_submission');
add_action('wp_ajax_nopriv_amp_submit_comment', 'tuancele_handle_amp_comment_submission');